...
### 2026-08-18 â€” Embedding model for duplicate detection
...
**Alternatives considered:** OpenAI `text-embedding-3-small` (1536 dims â€” would require
...
### 2026-08-18 â€” Duplicate similarity threshold
...
**Alternatives considered:** none yet â€” this is a placeholder; revisit once 20+ real
...
### 2026-08-22 â€” Drop computer vision, citizen selects category manually
...
override â€” rejected as extra UI/scope for marginal benefit given the timeline.
...
### 2026-08-22 â€” Add Docker for backend and citizen frontend
...
### 2026-08-22 â€” Zero-shot NLP via bart-large-mnli, severity only
...
**Alternatives considered:** Full summarization model (rejected â€” extra ~1GB+ weights and
latency); loading the pipeline at import time (rejected â€” slows container boot and breaks
any import-time test without weights).
**Impact:** `backend/app/services/nlp.py`, `backend/requirements.txt`
(transformers/torch added).

### 2026-08-22 â€” Storage layout: complaint-images/{year}/{month}/ via service-role key
...
uploads direct to Storage with anon key (rejected â€” bypasses backend validation of size/
...
### 2026-08-22 â€” Dedup: MiniLM embeddings + pgvector search exposed as SQL RPC
...
**Alternatives considered:** Direct psycopg/asyncpg connection (rejected â€” new driver,
new DATABASE_URL secret, drift from db.py); fetching all embeddings and computing
cosine in Python (rejected â€” defeats pgvector/indexing, won't scale).

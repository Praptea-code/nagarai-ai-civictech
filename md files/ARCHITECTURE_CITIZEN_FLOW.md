...
FastAPI (backend/app/routers/citizen.py)
   ...
   +-- AI pipeline (nlp.py, cv.py, embeddings.py)
...
| `backend/app/services/nlp.py` | `classify_complaint_text(text)` | Returns category, severity, summary |
| `backend/app/services/cv.py` | `detect_issue_in_image(image_bytes)` | Returns detected label + confidence |
...
```mermaid
flowchart TD
    A[Citizen submits: form + photo] --> B[FastAPI: create_complaint]
    B --> C[AI processing]
...
## Design notes
- NLP, CV, and embedding generation are independent of each other — run them
...
- If the CV model has low confidence (< 0.5) or no photo was submitted, don't force a
  category — rely on the NLP result instead.
